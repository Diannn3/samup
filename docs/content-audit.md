...
| UPLB Math Wizard | `src/data/events.ts`, `/events` | Repository event data | defined | keep, informational | No unsupported dates, national scope, or “brightest minds.” |
| Peer Tutorial Caravans | `src/data/events.ts`, `/resources` | Repository event data; Constitution objectives | defined | keep | Verified program/course context; schedule points to official updates. |
| STRASUC / Intramural Athletics | `src/data/events.ts` | Repository event data | defined | keep, informational | No invented venue/date. |
